...
// Outreach targets are NOT gaps and do not share their vocabulary. The backend
// (api/src/outreachStatus.ts, OUTREACH_STATUSES) accepts exactly these and
// rejects anything else with a 400 — "none" is its explicit clear. Pinned to
// the API list by src/outreach.test.ts; this constant once read GAP_STATES
// and every set_outreach_status call failed in production.
const OUTREACH_STATES = ["planned", "pitched", "placed", "declined", "none"];
...
const TOOL_DEFS = [
  ...
  },
  {
    name: "get_audit",
    description:
      "Get one AI-visibility audit in detail by scanId (from list_audits): overall AI-citation score + SEO/GEO/authority scores, per-engine citation summary (ChatGPT/Perplexity/Gemini/Claude/Grok/Google AI Overviews/Google AI Mode), the query×engine citation matrix, competitors cited, the top prioritised gaps (title, category, impact, effort), and pass/warn/fail audit check counts. In the citation matrix every engine appears explicitly per query with one of three states: \"cited\" (the engine's answer cited the brand), \"not_cited\" (the engine answered but did not cite the brand), or \"not_run\" (the engine produced no measured answer for that query — it sat the query out or errored; NOT a miss). Bulky raw fields (page HTML, full engine answers) are omitted. Requires a signed-in account.",
    inputSchema: {
      type: "object",
      properties: {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        engine: {
          type: "string",
          description:
            "Optional — limit to a single engine. One of: ChatGPT, Perplexity, Gemini, Claude, Grok, Google AI Overviews, Google AI Mode, or Google AI Mode (pass the key `aioverviews` or `aimode` for the last two; the others are their lowercase name, e.g. `chatgpt`). Google AI Overviews and Google AI Mode are DIFFERENT surfaces and routinely give different answers: asking for one never returns the other.",
        },
      },
  ...
  },
  {
    name: "set_prompts",
    description:
      "Edit the tracked prompts for a brand: the buyer questions the seven engines are asked every day. Add, remove, enable or disable them by exact text, or replace the whole set. Prompts are what the product actually measures, so this decides what the daily spend buys — a prompt no buyer types is a wasted engine check, and a prompt whose premise is false (\"is X the cheapest\") gets answered NO and scores as a citation anyway. Reads the current set first and returns a before/after diff, so an add never silently drops the prompts you did not mention. DESTRUCTIVE: `remove` deletes a prompt and `replace` overwrites the entire set, and a deleted prompt loses its place in the daily grid along with its history. Prefer `disable` when the goal is to stop spending on a prompt. WRITE ACTION — requires a Pro plan or higher; on Starter/trial the MCP is read-only.",
    inputSchema: {
      type: "object",
      properties: {
        brand: { type: "string", description: "Brand id (from list_brands) OR its bare domain, e.g. 'acme.com'." },
        add: {
          type: "array",
          items: { type: "string" },
          description: "Prompt texts to add. Existing prompts are kept. Duplicates (case-insensitive) are ignored rather than doubled.",
        },
        remove: {
          type: "array",
          items: { type: "string" },
          description: "Prompt texts to delete outright, matched case-insensitively against the current set. Deleting loses that prompt's future tracking; use `disable` to stop spending on it while keeping it.",
        },
        enable: { type: "array", items: { type: "string" }, description: "Prompt texts to switch back on." },
        disable: {
          type: "array",
          items: { type: "string" },
          description: "Prompt texts to stop checking without deleting. This is the reversible way to free pool capacity.",
        },
        replace: {
          type: "array",
          items: { type: "string" },
          description: "Replace the ENTIRE prompt set with exactly these. Destructive: every prompt not listed here is deleted, along with its place in the daily grid. Cannot be combined with add/remove/enable/disable. Prefer those unless the user asked for a full rewrite.",
        },
      },
      required: ["brand"],
      additionalProperties: false,
  ...
  },
  {
    ...
    description:
      "WHERE AI ANSWERS ABOUT YOUR CATEGORY ACTUALLY COME FROM, and whether you are really on those pages. Walks your recent scans and rolls up every third-party host the seven engines cited when answering your tracked prompts: how often it was cited, which rival brands the engines associate with it, which of your prompts it answers, and sample URLs. Then — the field that matters most — `youOnThisSite`, which is not inferred from the answers but comes from OPENING those pages and reading them: 'absent' means we read the page in full and your brand is not on it (that is the citation gap, `isGap: true`), 'mentioned' means you are named with no link back, 'linked' means the page links to you, 'unreachable' means the site refused an automated read, and 'unknown' means it has not been read yet. The last two are NOT evidence of absence and must never be reported as gaps. Pass `search` by brand name, host or URL fragment to answer which page or site a brand is and how many citations it has. Search returns `pages`, including rival-owned and other excluded pages; `excludedAs` says why a page is not a pitch target. Rival-owned and engine-owned domains are excluded from `targets`, because a mention on a competitor's own blog or in a search vendor's docs is not an opportunity. Also returns the pages of yours engines DO cite, so you can see what is already working, and `whatEachEngineReads`: per engine, how many answers cited you and the sites it leans on most. Pass `engine` to rank the targets for one engine's answers only (e.g. what Perplexity reads that ChatGPT does not). This is usually the highest-leverage list in the product and the one that explains a citation score that will not move: improving your own page is a different job from being present in the pages engines already read. Free, reads stored scan and page-check data. Requires a signed-in account.",
    inputSchema: {
      type: "object",
      properties: {
        domain: { type: "string", description: "Bare domain, e.g. 'acme.com'." },
        search: { type: "string", minLength: 1, maxLength: 100, description: "Find cited pages by brand name, host or URL fragment, e.g. marqeable or marqeable.com/blog." },
        range: { type: "string", enum: ["today", "7d", "30d", "all"], description: "The same window as the app's Sources screen. The API defaults to 30d." },
        gapsOnly: {
          type: "boolean",
        ...
        limit: { type: "integer", description: "Max targets and, with search, pages returned (1-100 each). Default 25. `totalTargets` counts every candidate host before this cut, so fewer targets than that means the list was truncated.", minimum: 1, maximum: 100 },
        offset: { type: "integer", description: "Skip this many targets, to page past the first `limit`: pass the `nextOffset` the previous call returned until it comes back null. Pages follow the citation ranking and never overlap. Default 0.", minimum: 0 },
        engine: {
          type: "string",
          enum: ["chatgpt", "perplexity", "gemini", "claude", "grok", "aioverviews", "aimode"],
          description: "Optional. Count targets and your cited pages from ONE engine's answers: chatgpt, perplexity, gemini, claude, grok, aioverviews (Google AI Overviews) or aimode (Google AI Mode). Omit for every engine. `whatEachEngineReads` always covers all of them.",
        },
      },
      required: ["domain"],
      additionalProperties: false,
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        status: { type: "string", description: "Where this target stands: planned, pitched, placed or declined. none clears a status you set by mistake.", enum: OUTREACH_STATES },
      },
  ...
  },
  {
    name: "gsc_status",
    description:
      "The authoritative Google Search Console connection check for this workspace: whether GSC OAuth is configured server-side, whether THIS workspace has connected (`connected`), and the verified GSC properties (site URLs / sc-domain: properties) available to query. A workspace can connect SEVERAL Google accounts — `connections[]` lists each one with its own email and properties, while `email` and `sites` summarise the first account and the union. A domain is readable when ANY connected account holds a property for it, so judge coverage from `sites`, not from one account. whoami does not report GSC state — this tool is the truthful signal.",
    inputSchema: { type: "object", properties: {}, additionalProperties: false },
  },
  {
    name: "list_brands",
  ...
  {
    name: "gsc_ai_searches",
    description:
      "The AI side of Search Console, as far as the API goes. Google's Search Analytics API has NO AI Overviews or AI Mode filter (`type` is web/image/video/news/discover/googleNews, and searchAppearance carries no AI value), and Google's own AI Overviews and AI Mode impression totals exist only in the Search Console UI's Generative AI report and its CSV export. But AI Mode searches ARE counted inside the ordinary web rows, and they are worded differently, so this reads the window's query+page rows and picks out the ones worded like prompts: follow_up (\"and how do i prove it\", \"what about poland\", \"spain?\"), chat_reply (\"yes please\", \"you are making it up\"), instruction (\"give me the cheapest one\", tracker prompts carrying answer directions), pasted_text (quiz and form questions, pasted paragraphs), agent_search (search operators, quoted fact-check probes, research keywords with \"official\" and a year) and long_prompt. Returns totals over named rows, the AI-style totals with their share of impressions, per-kind counts, and the top flagged rows by impressions with the reason each was flagged. PRECISION FIRST: ordinary long-tail searches are never flagged, so treat the count as a FLOOR on AI use, not a measurement of it, and never present it as Google's AI Overviews or AI Mode numbers. Named queries only (Google withholds rare ones). Cached for six hours. Requires a paid plan (Starter+).",
    inputSchema: {
      type: "object",
      properties: {
        domain: { type: "string", description: "Bare domain, e.g. 'example.com'. Must match a connected GSC property." },
        days: { type: "integer", description: "Trailing window in days (1-480). Default 90. Window ends ~2 days ago (GSC lag).", minimum: 1, maximum: 480 },
      },
      required: ["domain"],
      additionalProperties: false,
    },
...
// Tools that can DELETE or OVERWRITE user data, as opposed to appending or
// moving a status. set_prompts can drop a tracked prompt (`remove`) or rewrite
// the entire set (`replace`), and losing a prompt loses its place in the daily
// grid, so it earns the MCP destructiveHint that makes clients ask first.
const DESTRUCTIVE = new Set(["set_prompts"]);
const WRITE_TOOLS = new Set(["set_gap_status", "run_scan", "queue_article", "mark_article_published", "set_outreach_status", "set_prompts"]);
...
const TITLES: Record<string, string> = {
  ...
  set_prompts: "Edit the tracked prompts",
  ...
  gsc_ai_searches: "Find AI-style searches",
};
...
export const TOOLS = TOOL_DEFS.map((t) => {
  ...
  return {
    ...
    annotations: {
      ...
      // Most writes here add a queue entry, move a status or record a URL, and
      // are NOT destructive: claiming otherwise would train clients to warn
      // about the wrong things. set_prompts is the exception and is declared as
      // such, because `remove` deletes a tracked prompt and `replace` overwrites
      // the whole set. Clients that gate destructive tools behind a confirmation
      // should gate that one.
...
export const INSTRUCTIONS = `Cituna tracks how seven AI answer engines (ChatGPT, Perplexity, Gemini, Claude, Grok, Google AI Overviews, Google AI Mode) mention and cite brands, and joins that with Google Search Console (GSC).
...
- "which page/site is X, how many citations" -> list_citation_sources with search set to X.
...
- run_scan, set_gap_status, set_prompts, queue_article and mark_article_published need Pro or higher; on Starter this server is read-only, and the free trial has no MCP access at all.
- set_prompts decides what the daily spend buys. Two failure shapes are worth naming because both LOOK like wins: a prompt no buyer would ever type still costs seven engine checks a day, and a prompt whose premise is false ("is <brand> the cheapest X") gets answered NO while still scoring as a citation, because the brand was named. Read the answer text with get_engine_answers before trusting a citation on a branded prompt. Prefer 'disable' over 'remove' when freeing pool capacity: it is reversible and keeps the history.
...
// 2026-09-22: older APIs ignore q. Mirror citationSources.ts identity roots
// locally so a spaced brand name can find its host without an API import.
function sourceIdentityRoot(value: string): string {
  const t = value.toLowerCase().trim().replace(/^https?:\/\//, "").replace(/^www\./, "").split("/")[0];
  if (!t.includes(".")) return t.replace(/[^a-z0-9]/g, "");
  const parts = t.split(".").filter(Boolean);
  let i = parts.length - 2;
  if (i >= 1 && ["co", "com", "net", "org", "ac", "gov", "edu"].includes(parts[i])) i -= 1;
  return (i >= 0 ? parts[i] : parts[0] || "").replace(/[^a-z0-9]/g, "");
}

function sourceMatchesSearch(row: any, search: string): boolean {
  const q = search.toLowerCase();
  const root = sourceIdentityRoot(q);
  return [row.url, row.host, ...(row.brands ?? []), ...(row.rivals ?? [])]
    .some((v) => typeof v === "string" && v.toLowerCase().includes(q))
    || !!root && sourceIdentityRoot(String(row.host ?? "")) === root;
}
...
function summarizeChecks(checks: any): { pass: number; warn: number; fail: number } | null {
  ...
  for (const c of checks) {
    // An informational finding (the API's ai_training_access) is reported, not
    // passed or failed; the app's own counts leave it out, so these must too.
  ...
  server.setRequestHandler(CallToolRequestSchema, async (req) => {
    ...
    try {
      switch (name) {
        ...
        case "set_prompts": {
          const brandArg = String(args.brand ?? "").trim();
          if (!brandArg) return fail("`brand` is required — a brand id from list_brands, or its bare domain.");

          const list = (v: unknown): string[] =>
            Array.isArray(v) ? v.map((s) => String(s ?? "").trim()).filter(Boolean) : [];
          const add = list(args.add);
          const remove = list(args.remove);
          const enable = list(args.enable);
          const disable = list(args.disable);
          const replace = args.replace === undefined ? null : list(args.replace);

          if (replace && (add.length || remove.length || enable.length || disable.length)) {
            return fail("`replace` rewrites the whole set, so it cannot be combined with add/remove/enable/disable. Send one or the other.");
          }
          if (!replace && !add.length && !remove.length && !enable.length && !disable.length) {
            return fail("Nothing to change. Pass at least one of add, remove, enable, disable or replace.");
          }
          if (replace && !replace.length) {
            return fail("`replace` was empty, which would delete every tracked prompt for this brand. If that is really the intent, disable them individually instead.");
          }

          // Resolve id-or-domain against the workspace's own brands. A domain the
          // workspace does not track must fail here rather than 404 downstream:
          // the useful message is "you do not track that", not "not found".
          const projects = await api.get("/api/projects");
          const brands: any[] = Array.isArray(projects?.brands) ? projects.brands : Array.isArray(projects) ? projects : [];
          const wanted = normDomain(brandArg).toLowerCase();
          const match = brands.find(
            (b) => String(b?._id ?? "") === brandArg || normDomain(String(b?.domain ?? "")).toLowerCase() === wanted,
          );
          if (!match) {
            const known = brands.map((b) => String(b?.domain ?? "")).filter(Boolean);
            return fail(
              `No tracked brand matches "${brandArg}".` +
                (known.length ? ` This workspace tracks: ${known.join(", ")}.` : " This workspace tracks no brands yet — add one in the app."),
            );
          }
          const brandId = String(match._id);

          // Read-modify-write. The backend's PUT replaces the whole array, so an
          // `add` that posted only the new items would silently delete every
          // prompt the caller did not restate. Everything below is computed from
          // the CURRENT set for that reason.
          const cfg = await api.get(`/api/projects/${encodeURIComponent(brandId)}/config`);
          const before: Array<{ text: string; enabled: boolean }> = Array.isArray(cfg?.prompts)
            ? cfg.prompts.map((p: any) => ({ text: String(p?.text ?? "").trim(), enabled: p?.enabled !== false })).filter((p: any) => p.text)
            : [];

          const key = (s: string) => s.trim().toLowerCase();
          const notFound: string[] = [];
          let after: Array<{ text: string; enabled: boolean }>;
          ...
          if (replace) {
            ...
            for (const t of replace) {
              ...
              seen.add(key(t));
              after.push({ text: t, enabled: true });
            }
          } else {
            const byKey = new Map(before.map((p) => [key(p.text), { ...p }]));
            const removeSet = new Set(remove.map(key));
            for (const t of remove) if (!byKey.has(key(t))) notFound.push(t);
            ...
            after = [...byKey.values()].filter((p) => !removeSet.has(key(p.text)));
            for (const t of add) {
              if (after.some((p) => key(p.text) === key(t))) continue;
              after.push({ text: t, enabled: true });
            }
          }

          const saved = await api.put(
            `/api/projects/${encodeURIComponent(brandId)}/config`,
            { prompts: after },
          );
          const finalPrompts: Array<{ text: string; enabled: boolean }> = Array.isArray(saved?.prompts)
            ? saved.prompts.map((p: any) => ({ text: String(p?.text ?? ""), enabled: p?.enabled !== false }))
            : after;

          const beforeKeys = new Set(before.map((p) => key(p.text)));
          const afterKeys = new Set(finalPrompts.map((p) => key(p.text)));
          return ok({
            ok: true,
            brand: { id: brandId, domain: match.domain ?? null },
            counts: {
              before: before.length,
              after: finalPrompts.length,
              enabled: finalPrompts.filter((p) => p.enabled).length,
            },
            added: finalPrompts.filter((p) => !beforeKeys.has(key(p.text))).map((p) => p.text),
            removed: before.filter((p) => !afterKeys.has(key(p.text))).map((p) => p.text),
            // A prompt asked to change that was not in the set is reported rather
            // than swallowed: silently ignoring it reads as success and hides a typo.
            notFound: [...new Set(notFound)],
            prompts: finalPrompts,
            note: "Prompts are pooled workspace-wide, so the enabled count here counts against the plan pool. Changes take effect on the next scan.",
          });
        }
        ...
        case "competitor_discovery": {
          ...
          // What the app already knows each cited name IS to this brand
          // (GET /api/entity-roles): a university, a government body or a
          // publisher named in the answers is not a rival to add. The key is the
          // API's rival fold (competitorFilter.rivalKey), approximated for ASCII.
          const roleKey = (name: string) =>
            name.normalize("NFKD").replace(/[\u0300-\u036f]/g, "").toLowerCase()
              .replace(/\b(ai|io|app|hq|inc|labs)\b/g, "").replace(/[^\p{L}\p{N}]+/gu, "");
          const nameRoles = new Map<string, string>();
          try {
            const r = await api.get("/api/entity-roles", { domain });
            for (const rec of Array.isArray(r?.roles) ? r.roles : []) {
              if (rec?.kind === "name" && rec?.key && rec?.role) nameRoles.set(String(rec.key), String(rec.role));
            }
          } catch { /* no roles yet: every candidate stays, as before */ }
          let notRivals = 0;
            ...
            if (scanId) {
              ...
              if (Array.isArray(cited)) {
                ...
                for (const c of cited) {
                  ...
                  const role = nameRoles.get(roleKey(name));
                  if (role && role !== "competitor") { notRivals += 1; continue; }
          ...
          return ok({
            ...
            ...(sourcesRead.length < 2
              ...
              : {}),
            candidates,
            ...(notRivals ? { excludedNotCompetitors: notRivals } : {}),
            howToRead:
              "Each candidate appeared in your engine answers or your comparison searches and is NOT on your tracked competitor list. Names the app has already labelled as something other than a competitor (a publisher, a directory, a government body or university, a place) are left out and counted in `excludedNotCompetitors`. `evidence` weights an engine citation above a search token because a citation means an engine put them in front of your buyer. Names are extracted mechanically, so expect some noise: confirm before adding, then add via the app.",
        ...
        case "list_citation_sources": {
          ...
          const offset = args.offset != null ? Math.max(0, Math.trunc(Number(args.offset)) || 0) : 0;
          const engine = String(args.engine ?? "").trim().toLowerCase();
          const gapsOnly = args.gapsOnly === true;
          const search = typeof args.search === "string" ? args.search.trim() : "";
          if (args.search !== undefined && (typeof args.search !== "string" || !search || args.search.length > 100)) {
            return fail("search must be a string of 1-100 characters.");
          }
          const range = args.range;
          if (range !== undefined && (typeof range !== "string" || !["today", "7d", "30d", "all"].includes(range))) {
            return fail("range must be today, 7d, 30d or all.");
          }
          // The API picks the page: `limit` hosts from `offset` down the citation
          // ranking, or with gapsOnly down the verified gaps out of every candidate.
          // It used to send a fixed 25 whatever was asked, so `limit` above 25 did
          // nothing and nothing past the first page could be reached.
          const res = await api.get("/api/sources", {
            domain,
            q: search || undefined,
            range: range === undefined ? undefined : String(range),
            engine: engine || undefined,
            limit,
            offset: offset || undefined,
            gapsOnly: gapsOnly ? 1 : undefined,
          });
          // An API that predates paging ignores ?offset= and serves the first page
          // again. Handing that back as page two would repeat rows and a paging
          // loop would never end, so refuse instead.
          if (offset > 0 && res?.targetPage?.offset !== offset) {
            return fail("Paging through citation sources is not available on this backend yet. Try again in a few minutes, or raise `limit` (up to 100) instead.");
          }
          const outreach = res?.outreach ?? {};
          const returnedTargets = Array.isArray(res?.targets) ? res.targets : [];
          const returnedSources = Array.isArray(res?.sources) ? res.sources : [];
          // Without the query echo the API ignored q, so neither its rows nor
          // its targetCounts describe the search. Only count the local matches.
          const legacySearch = !!search && !res?.query;
          const all = legacySearch ? returnedTargets.filter((t: any) => sourceMatchesSearch(t, search)) : returnedTargets;
          const sources = legacySearch ? returnedSources.filter((s: any) => sourceMatchesSearch(s, search)) : returnedSources;
          const targetHosts = new Set(returnedTargets.map((t: any) => t.host));
          const pages = search ? [...sources].sort((a: any, b: any) => b.count - a.count).slice(0, limit).map((s: any) => ({
            url: s.url,
            host: s.host,
            citations: s.count,
            citationsByEngine: s.byEngine ?? {},
            engines: s.engines ?? [],
            firstSeen: s.firstSeen,
            lastSeen: s.lastSeen,
            isNew: s.isNew,
            rivalsNamedHere: s.brands ?? [],
            role: s.role ?? null,
            // Older APIs give no exclusion reason; target membership is the
            // only pitchability evidence available, not a guessed role verdict.
            pitchTarget: s.excludedAs === undefined ? targetHosts.has(s.host) : s.excludedAs === null,
            excludedAs: s.excludedAs ?? null,
            youOnThisPage: s.check?.presence ?? "unknown",
          })) : undefined;
          ...
          const targets = chosen.map((t: any) => ({
            ...
            isGap: isGap(t),
            // What the site is to this brand (publisher, directory, community…),
            // null until classified. Rival-owned, institutional and platform sites
            // are already excluded from targets.
            role: t.role ?? null,
            ...
            engines: t.engines ?? [],
            // Answers per engine that cited this host, across every engine.
            citationsByEngine: t.byEngine ?? {},
            yourPromptsItAnswers: t.queries ?? [],
            sampleUrls: t.urls ?? [],
            lastSeen: t.lastSeen,
            // "none" is the backend's own word for "no status recorded"; the old
            // default of "todo" taught callers gap vocabulary the write tool rejects.
            outreachStatus: outreach?.[t.host] ?? "none",
          }));
          // Counted by the API over EVERY candidate host before the page was cut, so
          // a short list and a truncated one can be told apart. An older API sends
          // no counts; counting what came back is the most it allows.
          const counts = legacySearch ? {} : res?.targetCounts ?? {};
          const count = (v: unknown, fallback: number) => (typeof v === "number" && Number.isFinite(v) ? v : fallback);
          const totalCount = count(counts.total, all.length);
          const gapCount = count(counts.absent, all.filter(isGap).length);
          const uncheckedCount = count(counts.unknown, all.filter((t: any) => !t?.presence || t.presence === "unknown").length);
          const pool = gapsOnly ? gapCount : totalCount;
          const end = offset + chosen.length;
          const nextOffset = end < pool ? end : null;
          const span = chosen.length === 1 ? `${end}` : `${offset + 1}-${end}`;
          let note: string | null = null;
          if (offset > 0 && !chosen.length) note = `Nothing past offset ${offset}: there are ${pool}.`;
          else if (nextOffset !== null) note = `Showing ${span} of ${pool}. Pass offset: ${nextOffset} for the next page${limit < 100 ? ", or raise `limit` (up to 100)" : ""}.`;
          else if (offset > 0) note = `Showing ${span} of ${pool}. That is the last page.`;
          if (legacySearch) note = [note, "Search only covered the API's top 200 pages for the range."].filter(Boolean).join(" ");
          return ok({
            domain,
            ...
            // The engine the targets were counted for, as the API applied it (an
            // unknown value falls back to every engine rather than erroring).
            engine: res?.engineFilter ?? null,
            offset,
            ...
            whatEachEngineReads: Array.isArray(res?.engineSummary)
              ? res.engineSummary.map((e: any) => ({
                  engine: e.engine,
                  answers: e.answers,
                  answersCitingYou: e.citedYou,
                  thirdPartyCitations: e.citations,
                  citationsOfYourPages: e.ownCitations,
                  topSites: e.sites ?? [],
                }))
              : [],
            ...(note ? { note } : {}),
            howToRead:
              ...
              (search ? " Search `pages` include rival-owned and other excluded sites; `excludedAs` explains why they are not pitch targets. `youOnThisPage` is the page check verdict. On older APIs the exclusion reason is unavailable and `pitchTarget` uses membership in the returned targets." : ""),
          });
        }
        ...
        case "set_outreach_status": {
          ...
          if (!OUTREACH_STATES.includes(status)) return fail(`\`status\` must be one of: ${OUTREACH_STATES.join(", ")}.`);
