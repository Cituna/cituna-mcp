...
// The backend's allowlist for OAuth connector tokens. A pure module with no
// imports, read here so a new tool cannot ship calling an endpoint that every
// Connectors user is refused (credentialPolicy.ts OAUTH_ROUTES).
import { oauthRoute } from "../../api/src/credentialPolicy.ts";
...
function mcpCalls(verb: "get" | "post" | "put"): string[] {
  const out: string[] = [];
...
test("every endpoint the MCP calls is open to an OAuth connector token", () => {
  // The backend refuses a connector token on any route not on its allowlist, so
  // a tool calling one works over an API key and fails for every Connectors user.
  const calls: Array<[string, string]> = [
    ...mcpCalls("get").map((p): [string, string] => ["GET", p]),
    ...mcpCalls("post").map((p): [string, string] => ["POST", p]),
    ...mcpCalls("put").map((p): [string, string] => ["PUT", p]),
    ["POST", "/api/scan"], // client.ts runScanStream, not written as api.post in tools.ts
    ["GET", "/api/auth/credential"], // http.ts pre-flight credential check
  ];
  assert.ok(calls.length >= 15, `only found ${calls.length} MCP calls — the scan is broken, not the code`);
  const refused = calls.filter(([m, p]) => !oauthRoute(m, p)).map(([m, p]) => `${m} ${p}`);
  assert.deepEqual(refused, [], `add these to OAUTH_ROUTES in api/src/credentialPolicy.ts: ${refused.join(", ")}`);
});
