...
test("exactly the tools that delete or overwrite are marked destructive", () => {
  // Most writes here append or move a status and are deliberately NOT
  // destructive. set_prompts is: `remove` drops a tracked prompt and `replace`
  // rewrites the whole set, and a lost prompt loses its place in the daily grid.
  // If this list grows, the tool's DESCRIPTION has to say so too — clients show
  // the description in the confirmation this hint triggers.
  const destructive = TOOLS.filter((t) => t.annotations.destructiveHint).map((t) => t.name).sort();
  assert.deepEqual(destructive, ["set_prompts"], `destructive set changed: ${destructive.join(", ")}`);
});

test("every destructive tool says so in its own description", () => {
  for (const t of TOOLS.filter((x) => x.annotations.destructiveHint)) {
    assert.match(
      t.description,
      /destructive|deleted|overwrit/i,
      `${t.name} is marked destructive but its description never warns the caller`,
    );
  }
});
