...
| Tool | Does |
|---|---|
| `whoami` | Confirm the connection, plan and usage. Start here. |
...
**"API key invalid or expired"** — regenerate under Integrations → Claude / MCP
access. If the key is definitely current, check `CITUNA_API_URL` has no trailing
path and is not an origin that redirects: a cross-origin redirect strips the
`Authorization` header, which surfaces as this exact message.
